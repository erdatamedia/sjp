...
class Box extends RestController
{
	...
	function index_get() {
		...
		$recordsTotal = $this->db->count_all('box');
		$recordsFiltered = $recordsTotal;
		if(isset($keyword) && $keyword['value'] != '') {
			$this->db->like('name', $keyword['value']);
			$recordsFiltered = $this->db->count_all_results('box');
		}
		...
		$data = $this->db->get('box')->result_array();
		$this->response(array(
			'draw' => $draw,
	...
	function save_post() {
		...
		$result = $this->db->insert('box', $data);
		if ($result) {
			$this->response([
				'status' 	=> $result,
				'msg' 		=> 'Saved',
	...
	function edit_post() {
		...
		$result = $this->db->where('id', $id)->get('box')->row_array();
	...
	function update_post() {
		$id = $this->input->post('id');
		$data['name']		= $this->input->post('name');
		$result = $this->db->where('id', $id)->update('box', $data);
		$this->response([
			'status' => $result,
			'msg'	 => 'Updated'
		]);
	...
	function delete_post() {
		...
		if (!$user) {
			$result = $this->db->where('id', $id)->delete('box');
			if ($result) {
				$this->response([
					'status' 	=> 1,
					'msg' 		=> 'Deleted',
	...
	private function checkBarang($id) {
		$data = $this->db->where('id_box', $id)->get('barang')->row_array();
		return $data;
	}
}
