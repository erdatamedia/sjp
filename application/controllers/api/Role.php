...
class Role extends RestController
{
	...
	function index_get() {
		...
		$recordsTotal = $this->db->count_all('role');
		$recordsFiltered = $recordsTotal;
		if(isset($keyword) && $keyword['value'] != '') {
			$this->db->like('name', $keyword['value']);
			$recordsFiltered = $this->db->count_all_results('role');
		}
	...
	function edit_post() {
		...
		$result = $this->db->where('id', $id)->get('role')->row_array();
	...
	function update_post() {
		$id = $this->input->post('id');
		$data['name']		= $this->input->post('name');
		$result = $this->db->where('id', $id)->update('role', $data);
		$this->response([
			'status' => $result,
			'msg'	 => 'Updated'
		]);
	...
	function delete_post() {
		...
		if (!$user) {
			$result = $this->db->where('id', $id)->delete('role');
			if ($result) {
				$this->response([
					'status' 	=> 1,
					'msg' 		=> 'Deleted',
	...
	private function checkUser($id) {
		$data = $this->db->where('id_role', $id)->get('user')->row_array();
		return $data;
	}
}
