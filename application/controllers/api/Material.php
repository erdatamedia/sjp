...
class Material extends RestController
{
	...
	function index_get() {
		...
		$recordsTotal = $this->db->count_all('material');
		$recordsFiltered = $recordsTotal;
		if(isset($keyword) && $keyword['value'] != '') {
			$this->db->like('name', $keyword['value']);
			$recordsFiltered = $this->db->count_all_results('material');
		}
	...
	function edit_post() {
		...
		$result = $this->db->where('id', $id)->get('material')->row_array();
	...
	function update_post() {
		$id = $this->input->post('id');
		$data['name']		= $this->input->post('name');
		$result = $this->db->where('id', $id)->update('material', $data);
		$this->response([
			'status' => $result,
			'msg'	 => 'Updated'
		]);
	...
	function delete_post() {
		...
		if (!$user) {
			$result = $this->db->where('id', $id)->delete('material');
			if ($result) {
				$this->response([
					'status' 	=> 1,
					'msg' 		=> 'Deleted',
