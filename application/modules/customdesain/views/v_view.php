			...
			<div class="btn-group btn-group-sm my-2 my-sm-0">
				...
				<?php if ($x['status'] == $status && $x['status'] != 'approved-shipping' && $x['status'] != 'waiting'):  ?>
					...
					<?php if ($user['id_role'] == 1 && $x['status'] == 'waiting'): ?>
						<button type="button" class="btn btn-warning btn-sm indicator_1" onclick="statusChangeRule(this, 'desain', 1)">
							<span class="indicator-label">
								Design
							</span>
							<span class="indicator-progress">
						...
						<!--<button type="button" class="btn btn-primary btn-sm indicator_2" onclick="statusChangeRule(this, 'cutting', 2)">-->
						<!--	<span class="indicator-label">-->
						<!--		Completed-->
						<!--	</span>-->
						<!--	<span class="indicator-progress">-->
						...
						<!--</button>-->
					<?php endif ?>
					<?php if ($x['status'] == 'printing' && $user['id_role'] == 4): ?>
						<button type="button" class="btn btn-danger btn-sm indicator_2" onclick="statusChangeCallback(this, 'desain', 2)">
							<span class="indicator-label">
								CallBack Design
							</span>
							<span class="indicator-progress">
					...
					<?php if ($user['id_role'] == 3): ?>
						<a target="_blank" href="<?= base_url('prints/design/'). $x['id']?>" class="btn btn-light">Draf Design</a>
					<?php endif ?>
					<?php if ($user['id_role'] == 3): ?>
						<a target="_blank" href="<?= base_url('prints/color/'). $x['id']?>" class="btn btn-light">Draf Pantone Color</a>
					<?php endif ?>
			</div>
								...
								<?php if ($line['barang']): ?>
										...
										<div class="mb-2 col-md-10">
											...
											<div class="d-flex">
												<a href="<?= base_url('customdesain/upload/') . $line['id_barang'] . '/' . $x['id'] ?>" class="btn btn-primary btn-sm">
													Design Documents
													<?php if ($line['barang']['inside'] && $line['barang']['outside'] && $line['barang']['color']): ?>
														3/3
													<?php elseif ($line['barang']['inside'] && $line['barang']['color']): ?>
														2/3
													<?php elseif ($line['barang']['outside'] && $line['barang']['color']): ?>
														2/3
													<?php elseif ($line['barang']['outside'] && $line['barang']['inside']): ?>
														2/3
													<?php elseif ($line['barang']['inside'] || $line['barang']['outside'] || $line['barang']['color']): ?>
														1/3
													<?php else: ?>
														0/3
													<?php endif; ?>
											...
											<textarea class="form-control form-control-sm mb-2" name="deskripsi" <?= $disabled ?> data-kt-autosize="false"><?= $line['deskripsi'] ? $line['deskripsi'] : 'Empty' ?></textarea>
										</div>
									</td>
									<td>
									...
									<td>
										...
										<?php elseif($line['qty_keluar']): ?>
											<label class="form-label">Qty Out</label>
											<div class="fw-bolder"><?= $line['qty_keluar']? $line['qty_keluar'] : 0 ?></div>
										</td>
									<?php endif ?>
								</tr>
								<?php endif ?>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="text-center mb-5">
					<span>
						Harap Masukan Note untuk pekerjaan <?= $x['id_pesanan'] ?>
					</span>

				</div>
				<textarea class="form-control note"></textarea>
				<div class="text-center pt-15">
					<button type="button" onclick="saveCancelNote(this)" class="btn btn-light me-3">Close</button>
