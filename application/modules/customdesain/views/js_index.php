...
<script type="text/javascript">
	...
	const populateTemplate = (data, target) => {
		...
		data.forEach((d, index) => {
            ...
            const image = newTemplate.querySelector('[data-kt-subtable="template_image"]');
			...
			if (image) {
                const outside = d.outside || defaultImage;
                const inside = d.inside || defaultImage;

                image.innerHTML = `
                    <div class="d-flex gap-3">
                        <div class="d-flex flex-column align-items-center">

                            <a class="overlay symbol symbol-50px bg-secondary bg-opacity-25 rounded" data-fslightbox="lightbox-basic" href="${inside}">
                                <div class="symbol-label" style="background-image:url('${inside}')"></div>
                                <div class="overlay-layer bg-dark bg-opacity-25">
                                    <i class="bi bi-zoom-in text-white"></i>
                                </div>
                            </a>
                            <label class="form-label fs-7">Inside</label>
                        </div>
                        <div class="d-flex flex-column align-items-center">

                            <a class="overlay symbol symbol-50px bg-secondary bg-opacity-25 rounded" data-fslightbox="lightbox-basic" href="${outside}">
                                <div class="symbol-label" style="background-image:url('${outside}')"></div>
                                <div class="overlay-layer bg-dark bg-opacity-25">
                                    <i class="bi bi-zoom-in text-white"></i>
                                </div>
                            </a>
                             <label class="form-label fs-7">Outside</label>
                        </div>
                    </div>
	...
	function validateImage(imageUrl, callback) {
        if (!imageUrl) {
        ...
        img.onload = () => callback(imageUrl); // Jika gambar valid, gunakan gambar asli
        img.onerror = () => callback(defaultImage); // Jika gagal, gunakan gambar default
    }
			...
			drawCallback: function(settings) {
				...
				$('.count').each((i, j) => {
					if (i > 0) {
						...
						const count = settings['json']['count'][statuss]
			...
			columns: [{
				...
				render: function (data, type, row) {
					...
					if (id_role_user == 2) {
						...
						<a href="<?= base_url('customdesain/form/') ?>${data}" class="btn btn-icon btn-light btn-active-light-success"><i class="bi bi-pencil"></i></a>
						<a href="<?= base_url('customdesain/view/') ?>${data}" class="btn btn-icon btn-light btn-active-light-success"><i class="bi bi-eye"></i></a>
			...
			buttons: {
				buttons: [
				{
					...
					action: function (e, dt, node, config) {
						window.location = '<?= base_url('customdesain/form') ?>'
					}
				}, {
