...
class Polosan extends MY_Controller
{
	...
	private function getPekerjaan($id) {
		$result = $this->db->select('pekerjaan.id, pekerjaan.id_user, pekerjaan.created_at, pekerjaan.due_date, pekerjaan.tgl_pengiriman, pekerjaan.id_pesanan, pekerjaan.jenis_order, pekerjaan.durasi, pekerjaan.status, pekerjaan.design_type, pekerjaan.design_attachment, pekerjaan.completed_at, pekerjaan.shipped_at, user.name, user.image')->join('user', 'user.id=pekerjaan.id_user', 'left')->where('pekerjaan.id', $id)->get('pekerjaan')->row_array();
		if ($result) {
