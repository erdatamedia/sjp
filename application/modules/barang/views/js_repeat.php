	...
	document.addEventListener("DOMContentLoaded", function () {
		...
		const populateTemplate = (data, target) => {
			data.forEach(d => {
				...
				var link = ''
				if (d.jenis_pekerjaan === 'custom-desain') {
					link = "<?= base_url('customdesain/view/') ?>" + d.id
				} else {
					link = "<?= base_url('polosan/view/') ?>" + d.id
				}
		...
		table = $('#table').DataTable({
			...
			columns: [{
				...
				title: 'No.MC', data: 'barang.no_mc', className: 'min-w-125px',
			}, {
				title: 'Product Name', data: 'barang.item_box', className: 'min-w-125px',
			}, {
