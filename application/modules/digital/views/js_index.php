...
<script type="text/javascript">
	...
	const populateTemplate = (data, target) => {
		...
		data.forEach((d, index) => {
			...
			const image_elm = newTemplate.querySelector('[data-kt-subtable="template_image"]');
			const finishing = newTemplate.querySelector('[data-kt-subtable="template_description"]')
			...
			if (image_elm) {
                const image = d.image || defaultImage;
    
                image_elm.innerHTML = `
                    <div class="d-flex gap-3">
                        <div class="d-flex flex-column align-items-center">
                            
                            <a class="overlay symbol symbol-50px bg-secondary bg-opacity-25 rounded" data-fslightbox="lightbox-basic" href="${image}">
			...
			columns: [{
				...
				render: function (data, type, row) {
					...
					if (id_role_user == 2) {
						...
						<a href="<?= base_url('digital/form/') ?>${data}" class="btn btn-icon btn-light btn-active-light-success"><i class="bi bi-pencil"></i></a>
						<a href="<?= base_url('digital/view/') ?>${data}" class="btn btn-icon btn-light btn-active-light-success"><i class="bi bi-eye"></i></a>
			...
			buttons: {
				buttons: [
				{
					...
					action: function (e, dt, node, config) {
						window.location = '<?= base_url('digital/form') ?>'
					}
				}, {
