...
class RestController extends \CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->session->userdata('sbki')) {
            $this->user = $this->db->where('id', $this->session->userdata('sbki')['id'])->get('user')->row_array();
        }
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        // Dilewati jika: ENVIRONMENT=development, APP_ENV=local, atau request dari localhost/127.0.0.1
        $is_local_request = in_array(
            $_SERVER['REMOTE_ADDR'] ?? '',
            ['127.0.0.1', '::1', 'localhost']
        ) || (isset($_SERVER['HTTP_HOST']) && preg_match('/^localhost(:\d+)?$/', $_SERVER['HTTP_HOST']));

        $skip_https = $is_local_request
            || (defined('ENVIRONMENT') && ENVIRONMENT === 'development')
            || (getenv('APP_ENV') === 'local');

        if ($this->config->item('force_https') && $this->request->ssl === false && !$skip_https) {
            $this->response([
                $this->config->item('rest_status_field_name')  => false,
